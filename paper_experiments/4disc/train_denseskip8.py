...
dense4 = lasagne.layers.DenseLayer(dense3, num_units = HIDDEN)
dense5 = lasagne.layers.DenseLayer(dense4, num_units = HIDDEN)
dense6 = lasagne.layers.DenseLayer(dense5, num_units = HIDDEN)
dense7 = lasagne.layers.DenseLayer(dense6, num_units = HIDDEN)
dense8 = lasagne.layers.DenseLayer(dense7, num_units = VARS*SITES, nonlinearity = None)

ressum = lasagne.layers.ElemwiseSumLayer([reshape, dense8])
...
for epoch in range(20000):
	...
	f = open("denseskip8_error.txt","a")
	f.write("%d %.6g\n" % (epoch,err))
	f.close()
	
	pickle.dump(lasagne.layers.get_all_param_values(output),open("denseskip8_network.params","wb"))
