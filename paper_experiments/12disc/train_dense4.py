...
output = lasagne.layers.ReshapeLayer(dense4,(-1,VARS,SITES))
...
for epoch in range(20000):
	...
	f = open("dense4_error.txt","a")
	f.write("%d %.6g\n" % (epoch,err))
	f.close()
	
	pickle.dump(lasagne.layers.get_all_param_values(output),open("dense4_network.params","wb"))
